...
@section('content')
    ...
    <script>
        ...
        @php
            $existingColorImagesData = $product->images
                ->where('media_type', 'image')
                ->whereNotNull('color')
                ->where('is_color_thumb', '!=', true)
                ->groupBy('color')
                ->map(function ($imgs) {
            ...
            $existingColorThumbsData = $product->images
                ->where('is_color_thumb', true)
                ->whereNotNull('color')
                ->mapWithKeys(function ($img) {
                    return [$img->color => media_url($img->path)];
                });
        @endphp
        const existingColorImages = @json($existingColorImagesData);
        const existingColorThumbs = @json($existingColorThumbsData);
        ...
        function addColorQuantityRow(color = '', size = '', quantity = '') {
            ...
            // Existing color thumbnail (small color-button picture)
            const existingThumb = (color && existingColorThumbs[color]) ? existingColorThumbs[color] : '';
            ...
            row.innerHTML = `
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                    ...
                    <input type="text" class="input" name="color_name_${index}" placeholder="Color name" value="${colorName}" style="flex:1;padding:6px;font-size:1rem;min-width:120px;">
                    <input type="text" class="input" name="size_${index}" placeholder="Size (S, M, L, XL, 42)" value="${size}" style="padding:6px;font-size:1rem;width:130px;">
                    <input type="number" class="input" name="quantity_${index}" placeholder="Qty" min="1" value="${quantity}" style="padding:6px;font-size:1rem;width:80px;">
                ...
                <div style="display:grid;grid-template-columns:1fr;gap:6px;">
                    ...
                    <input type="number" class="input" name="price_${index}" placeholder="Uses base price if blank" step="0.01" min="0" value="${priceVal}" style="padding:6px;font-size:1rem;">
                </div>
            `;
            ...
            if (existingThumb) {
                ...
                if (thumb) {
                    thumb.innerHTML = '<img src="' + existingThumb + '" style="width:100%;height:100%;object-fit:cover;display:block;">';
                    thumb.style.border = '1px solid #d1d5db';
                }
            }
        }

        // Preview the color thumbnail (small color-button picture)
        ...
        // Preview per-color gallery images
