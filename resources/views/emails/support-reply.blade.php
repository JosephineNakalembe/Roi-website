    ...
    <table role="presentation" style="width: 100%; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; margin-top: 20px; margin-bottom: 20px;">
        <tr>
            <td style="padding: 24px; background: linear-gradient(135deg, #2563eb, #1d4ed8);">
                <h1 style="color: #ffffff; margin: 0; font-size: 22px;">Support Ticket Update</h1>
                <p style="color: rgba(255,255,255,0.85); margin: 6px 0 0; font-size: 14px;">{{ $ticket->subject }}</p>
            </td>
        </tr>
        <tr>
            <td style="padding: 24px;">
                ...
                <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 12px; padding: 16px; margin-bottom: 16px;">
                    ...
                    <div style="margin-bottom: 12px; text-align: right;">
                        <div style="display: inline-block; background: #2563eb; color: #fff; padding: 10px 14px; border-radius: 12px 12px 4px 12px; font-size: 14px; max-width: 80%;">
                            <p style="margin: 0; font-size: 11px; opacity: 0.7;">You</p>
                            <p style="margin: 4px 0 0;">{{ $ticket->message }}</p>
                            <p style="margin: 4px 0 0; font-size: 11px; opacity: 0.7;">{{ $ticket->created_at->format('M d, H:i') }}</p>
                        </div>
                    </div>
                        ...
                        @foreach($allReplies as $reply)
                            @if($reply['sender'] === 'admin')
                                <div style="margin-bottom: 12px; text-align: left;">
                                    <div style="display: inline-block; background: #f3f4f6; color: #111; padding: 10px 14px; border-radius: 12px 12px 12px 4px; font-size: 14px; max-width: 80%;">
                                        <p style="margin: 0; font-weight: 600; font-size: 11px; color: #6b7280;">Admin</p>
                                        <p style="margin: 4px 0 0;">{{ $reply['message'] }}</p>
                                        <p style="margin: 4px 0 0; font-size: 11px; color: #9ca3af;">{{ \Carbon\Carbon::parse($reply['created_at'])->format('M d, H:i') }}</p>
                                    </div>
                                </div>
                            @else
                                <div style="margin-bottom: 12px; text-align: right;">
                                    <div style="display: inline-block; background: #2563eb; color: #fff; padding: 10px 14px; border-radius: 12px 12px 4px 12px; font-size: 14px; max-width: 80%;">
                                        <p style="margin: 0; font-size: 11px; opacity: 0.7;">You</p>
                                        <p style="margin: 4px 0 0;">{{ $reply['message'] }}</p>
                                        <p style="margin: 4px 0 0; font-size: 11px; opacity: 0.7;">{{ \Carbon\Carbon::parse($reply['created_at'])->format('M d, H:i') }}</p>
                                    </div>
                                </div>
                            @endif
                        @endforeach
                    ...
                    @if($latestAdminReply)
                        <div style="text-align: left; padding: 12px; background: #fefce8; border: 1px solid #fde68a; border-radius: 8px; margin-top: 8px;">
                            <p style="margin: 0; font-size: 12px; color: #92400e; font-weight: 600;">⬆ Latest Reply</p>
                        </div>
                    @endif
                </div>
